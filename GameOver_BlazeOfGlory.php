...
if ($id <> '') {
    ...
    if ($high_score == '') {
        $player_update = mysql_query("UPDATE player_sheet SET (isZombie, game_over_datetime) VALUES (2, '$gameover_datetime') WHERE id='$id'") or die(mysql_error());
    } else {
        $player_update = mysql_query("UPDATE player_sheet SET (isZombie, game_over_datetime, high_score) VALUES (2, '$gameover_datetime', '$high_score') WHERE id='$id'") or die(mysql_error());
    }
