...
if ($id <> '') {
    ...
    if ($high_score == '') {
        $player_update = $mysqli->query("UPDATE player_sheet SET isZombie=2, game_over_datetime='$gameover_datetime' WHERE id='$id'") or die($mysqli->error());
    } else {
        $player_update = $mysqli->query("UPDATE player_sheet SET isZombie=2, game_over_datetime='$gameover_datetime', high_score='$high_score' WHERE id='$id'") or die($mysqli->error());
    }
